# Breach 19 — Insecure Design — *no flag*
...
- **MD5 used as if it were a secret.** The password-reset token is
  ...
  (breach 13). MD5 is a *fast, unsalted, public* function — using it anywhere a
...
- **No anti-automation anywhere in the auth design.** There is no rate-limiting,
  lockout, or CAPTCHA on login or on the reset flow (see breach 18). The design
...
- **Secrets reachable by design.** An internal config endpoint returns the JWT
  ...
  and writable fields are published at `/api/docs-internal` (breach 15). The
