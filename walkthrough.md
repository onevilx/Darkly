...
> The subject requires, for the bonus, **5 more vulnerabilities** on top of the 10 mandatory ones (one of which — at least — exposes no flag). The nine weaknesses numbered 11-19 below were all confirmed live against `localhost:4942` / `localhost:8090`, as distinct instances. None of them are needed to become administrator, and (in this build) none of them mint a new `FLAG{...}` token — they are the "weaknesses that expose no flag" the subject talks about. Together with the ten flagged breaches above, they bring the audit to **19 explained vulnerabilities — exactly the subject's stated platform-wide total**. Vulnerability 20, "Insecure Design", is on top of that count: it doesn't add a new instance, it synthesizes several of the ones above (04, 07, 14, 16, 19) through a design-level lens.
...
So in the end i got **10 flags** (6 mandatory + 4 bonus) and explained **19 vulnerabilities** (10 mandatory + 9 bonus), which matches the subject's own stated total of 19 vulns on the platform exactly. The subject only requires explaining 10 of them for mandatory + 5 more for bonus (15 minimum), so 19 clears that with room to spare on its own. On top of those 19 there's a 20th write-up, "Insecure Design" — it doesn't introduce a new bug, it steps back and names the design-level pattern shared by breaches 04, 07, 14, 16 and 19, so i'm not counting it as a 20th distinct vulnerability, just an extra.
